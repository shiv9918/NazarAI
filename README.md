...
NazarAI's end-to-end workflow connects citizens, AI, and municipal teams into a seamless complaint resolution pipeline.

### 1️⃣ Citizen Complaint Submission

```mermaid
flowchart LR
    A(["👤 Citizen"]) -->|"Opens Web Portal\nor WhatsApp"| B["📝 Submit Report"]
    B -->|"Uploads photo +\nGPS location"| C["🖼️ Image + Location\nCaptured"]
...
### 2️⃣ AI Classification & Routing Pipeline

```mermaid
flowchart TB
    A(["⚙️ Backend receives report"]) --> B["📷 OpenCV\nImage Pre-processing"]
    B --> C["🧠 Google Gemini\nImage & Text Analysis"]
    C --> D{"🔍 Classification\nResult"}
    D -->|"Issue Type"| E["🏷️ e.g. Pothole / Garbage\n/ Water Leakage / Wires"]
    D -->|"Urgency Level"| F["🚨 Low / Medium /\nHigh / Critical"]
    D -->|"Department"| G["🏢 PWD / Sanitation /\nWater / Electricity"]
    E & F & G --> H["🔄 Duplicate\nDetection"]
    H -->|"Unique"| I["💾 Saved to PostgreSQL"]
    H -->|"Duplicate"| J["🔁 Merged with\nexisting complaint"]
    I --> K["📲 WhatsApp confirmation\nsent to citizen"]
...
### 3️⃣ Municipal Resolution Lifecycle

```mermaid
stateDiagram-v2
    [*] --> Reported : Citizen submits complaint
    Reported --> InProgress : Department acknowledges
    InProgress --> Resolved : Work completed
    Resolved --> FeedbackRequested : System triggers feedback
    FeedbackRequested --> Closed : Citizen rates resolution
    Resolved --> Reopened : Citizen rejects resolution
    Reopened --> InProgress : Re-assigned to department
    Closed --> [*]
```

### 4️⃣ WhatsApp Workflow
...
sequenceDiagram
    ...
    Citizen->>WA: Send photo + location
    WA->>Twilio: Inbound message
    Twilio->>API: POST /api/whatsapp/webhook
    API->>AI: Analyze image
    AI-->>API: Classification + urgency
    API->>DB: Store complaint
    API->>Twilio: Send confirmation
    Twilio->>WA: Complaint ID + status
    WA-->>Citizen: "Complaint #123 registered!"

    Note over Citizen,DB: Status updates sent automatically
    DB-->>API: Status changed
    API->>Twilio: Notify citizen
    Twilio->>WA: Status update
    WA-->>Citizen: "Your complaint is In Progress"
```

### 5️⃣ Weather Alert Workflow

```mermaid
flowchart LR
    A["🌦️ OpenWeather API\n48-hr forecast"] -->|"Rainfall threshold\nexceeded"| B["⚠️ Alert triggered"]
    B --> C["📊 Municipal\nDashboard notified"]
    B --> D["🏢 Department\nAlert Center"]
    D --> E["📲 Proactive SMS/\nWhatsApp to team"]
    C --> F["📋 Pre-position\nresources"]
...
### 6️⃣ Role-Based Access Summary

| Role | Access | Actions |
|:---|:---|:---|
| 👤 **Citizen** | Web Portal + WhatsApp | Submit, track, rate complaints |
| 🏢 **Department** | Department Panel | View assigned complaints, update status |
| 🏛️ **Municipal** | Municipal Dashboard | Full oversight, analytics, alerts |
| 🔑 **Admin** | All panels | User management, system config |
